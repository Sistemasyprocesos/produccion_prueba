...
/* ===================================
   1. VALIDAR FECHAS (ANTES DE GUARDAR)
=================================== */
...
foreach ($real as $secuencia => $turnos) {
    ...
    foreach ($turnos as $turno => $valor) {
        ...
        $f = !empty($f_check) ? $f_check : null;
        $j = !empty($j_check) ? $j_check : null;
        $h = intval($h_check);
        $p = floatval($peso[$secuencia][$turno] ?? 0);
        $e = floatval($eq[$secuencia][$turno] ?? 1);
        $obj_kg = floatval($obj_check);
