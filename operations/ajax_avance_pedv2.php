...
$std_base = $actividades[0]['kg_std'] ?? 1;
...
<td>
    <input type="number" class="form-control" onkeypress=soloNumeros(event) name="turno[]" value="<?= $f['turno'] ?>">
</td>

<td>
<select class="form-select" name="fase[]">
    <?php foreach($actividades as $act): ?>
        <option value="<?= $act['secuencia'] ?>" <?= $f['secuencia']==$act['secuencia']?'selected':'' ?>>
        <?= $act['etapanombre'] ?>
        </option>
    <?php endforeach; ?>
</select>
...
<td></td>
<td>
<input type="text" class="form-control" value="<?= $f['kg_real'] ?>" onkeypress="soloNumeros(event)" name="real[]">
</td>
...
const actividades = <?= $actividadesJS ?>;

/* ======================
CONSTRUIR SELECT
======================*/

const opcionesSelect = actividades.map(a =>
`<option value="${a.secuencia}">${a.etapanombre}</option>`
).join('');
...
<td>
<input type="number" class="form-control" name="turno[]" value="${ultimoTurno+1}">
</td>
...
+
<td>
<input type="text" class="form-control" name="real[]" onkeypress="soloNumeros(event)">
</td>
...
<td>
<input type="number" class="form-control" name="hc[]" min="0">
</td>
